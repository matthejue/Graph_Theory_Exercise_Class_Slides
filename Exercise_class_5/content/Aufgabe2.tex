...
\begin{frame}[t]{Aufgabe \thesection}{\small Maximum Matching, Minimum Vertex Cover, Maximum Indepedant Set, Minimum Edge Cover\vspace{0.5cm}}
  \begin{columns}
    ...
    \begin{column}[t]{0.25\textwidth}
      ...
      \begin{itemize}
        \item $\alpha(B) = 7$\only<2>{\footnote[4]{\tiny the complement of a maximum independent set is a minimum vertex cover.}}
      \end{itemize}
    \end{column}
    \begin{column}[t]{0.25\textwidth}
      \resizebox{\textwidth}{!}{
      ...
      \begin{itemize}
        \item $\beta'(B) = 7$\only<2>{\footnote[4]{\tiny finding a maximum matching and extending it greedily so that all vertices are covered}}
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
